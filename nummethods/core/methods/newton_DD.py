...
def solve_newton_diferencias_divididas(file, interpolation_value):
    ...
    if len(np.unique(x)) != len(x):
        raise ValueError("Los valores de x no deben repetirse para interpolación de Newton")
    ...
    for j in range(1, n):
        for i in range(n - j):
            ...
            if denominator == 0:
                raise ValueError("Se encontró división entre cero en diferencias divididas")
